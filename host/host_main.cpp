/*
 * Desktop entry: init SDL, then run the Atari 2600 core (app_main_a2600).
 */
...
{
    const char *title = "Atari 2600 / Stella 2014 (host)";
    ...
    std::printf("host: expects stella2014.bin in cwd (make / make pack)\n");
    if (rom)
        std::printf("host: ROM %s\n", rom);
    else
        std::printf("host: no ROM — pass path as argv[1] or set HOST_ROM\n");

    app_main_a2600(0, 0, -1);
