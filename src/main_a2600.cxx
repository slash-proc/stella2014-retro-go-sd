/* Atari 2600 (stella2014) standalone core — talks to the firmware only
 * through gw_firmware_abi_t. Include gw_core_bridge.h last so ACTIVE_FILE /
 * common_emu_state rewrite to live ABI pointers. */
extern "C"
{
...
#ifdef HOST_BUILD
#define DEFPROPS_CORE_FALLBACK "stella2014.bin"
#else
#define DEFPROPS_CORE_FALLBACK "/cores/stella2014.bin"
#endif

static const char *defprops_core_path(void)
{
    if (ACTIVE_FILE && ACTIVE_FILE->system && ACTIVE_FILE->system->core_path &&
        ACTIVE_FILE->system->core_path[0]) {
        return ACTIVE_FILE->system->core_path;
    }
    return DEFPROPS_CORE_FALLBACK;
}
...
void fill_stella_config(string md5string)
{
    ...
    const char *core_bin = defprops_core_path();
    if (!defprops_init(core_bin)) {
        printf("Stella: Failed to load database (%s).\n", core_bin);
        strcpy(a2600_controll, "");
        strcpy(a2600_controlr, "");
...
/* CORE_ENTRY — gw_core_entry.S runs .init_array before branching here. */
extern "C" void app_main_a2600(uint8_t load_state, uint8_t start_paused, int8_t save_slot)
{
    ...
    /* ITCM holds hot .text (stella2014_core.ld) — do not use it as a C++ heap. */
