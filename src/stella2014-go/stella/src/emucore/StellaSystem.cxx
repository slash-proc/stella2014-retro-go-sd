...
#ifdef TARGET_GNW
  /* PageAccess is touched on every peek/poke — DTCM first; fall back to
   * RAM_EMU. TIA FBs (~100 KiB) also prefer DTCM; together they fit the
   * ~104 KiB core pool with a little slack. */
  myPageAccessTable = (PageAccess *)dtc_calloc(myNumberOfPages, sizeof(PageAccess));
  if(!myPageAccessTable)
    myPageAccessTable = (PageAccess *)ram_calloc(myNumberOfPages, sizeof(PageAccess));
  myPageIsDirtyTable = (bool *)dtc_calloc(myNumberOfPages, sizeof(bool));
  if(!myPageIsDirtyTable)
    myPageIsDirtyTable = (bool *)ram_calloc(myNumberOfPages, sizeof(bool));
#else
...
#ifdef TARGET_GNW
#ifdef HOST_BUILD
  free(myPageAccessTable);
  free(myPageIsDirtyTable);
#endif
  myPageAccessTable = 0;
  myPageIsDirtyTable = 0;
#else
