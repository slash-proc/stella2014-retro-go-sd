...
#ifdef TARGET_GNW
  /* Prefer DTCM for the hot TIA write path; fall back to RAM_EMU.
   * ITCM is reserved for hot .text (see stella2014_core.ld) — not heap data. */
  myCurrentFrameBuffer = (uInt8 *)dtc_malloc(160 * 320);
  if(!myCurrentFrameBuffer)
    myCurrentFrameBuffer = (uInt8 *)ram_malloc(160 * 320);
  myPreviousFrameBuffer = (uInt8 *)dtc_malloc(160 * 320);
  if(!myPreviousFrameBuffer)
    myPreviousFrameBuffer = (uInt8 *)ram_malloc(160 * 320);
  if(myCurrentFrameBuffer)
...
{
#ifdef TARGET_GNW
  /* DTCM/RAM_EMU bumps have no free — host dtc_malloc is malloc, so free. */
#ifdef HOST_BUILD
  free(myCurrentFrameBuffer);
  free(myPreviousFrameBuffer);
#endif
  myCurrentFrameBuffer = 0;
  myPreviousFrameBuffer = 0;
#else
